...
2. **Builder:** **Nixpacks only** — this repo has **no `Dockerfile`**. In the service **Settings → Build**, confirm builder is **Nixpacks** (not Dockerfile / Railpack). **`railway.toml`** + **`railway.json`** set `"builder": "NIXPACKS"` and `"dockerfilePath": null`.
3. Use **`bash start.sh`** — worker in background, Flask dashboard in foreground; health check **`/health`**.
4. **Build:** **`runtime.txt`** (`python-3.11.0`) + **`pip install --no-cache-dir -r requirements-deploy.txt`** (CPU-only `torch`). Commit **`.dockerignore`** / **`.gitignore`** so context excludes **`.venv`**, **`persist/`**, **`quantbot/`**, tests, and DBs. After a failed build, **redeploy with “Clear build cache”**.
