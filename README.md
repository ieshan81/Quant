...
2. **Builder:** **Nixpacks** — this repo has **no `Dockerfile`** and **no `railpack.json`**. **`railway.toml`** / **`railway.json`** set `"builder": "NIXPACKS"` with **`nixpacks.toml`**. If the Railway UI ignores `NIXPACKS` in config-as-code, set **Builder → Nixpacks** manually in service settings.
3. Use **`bash start.sh`** — worker in background, Flask dashboard in foreground; health check **`/health`**.
4. **Build:** Python **3.11** + **`pip install --no-cache-dir -r requirements-deploy.txt`** (lean; no torch/transformers). Optional local ML: `pip install -r requirements-ml.txt`. Commit **`.dockerignore`** so **`requirements-deploy.txt`** is in the build context.
