...
**Railway.app:** Treat the **`quantbot/`** folder as the Git repo root (or set Railway **Root Directory** to `quantbot`). **`railway.json`** is for the **dashboard** service (Nixpacks, `python monitoring/dashboard.py`, `/health`). **`railway.worker.json`** is for the **worker** (`python main_worker.py`); in Railway, point the worker service at that file under **Settings → Config as code** (or set **Custom Start Command** to `python main_worker.py`). **`requirements-deploy.txt`** pins **CPU-only** `torch` (set **Custom Install Command** to `pip install -r requirements-deploy.txt` if Nixpacks defaults pull the wrong torch). Share **`QUANTBOT_DB_PATH`** on a **volume** (e.g. `/app/data/quantbot.sqlite3`) between both services. Also **`runtime.txt`**, **`Procfile`**.
