...
**Railway.app:** Treat the **`quantbot/`** folder as the Git repo root (or set Railway **Root Directory** to `quantbot`). Use **`requirements-deploy.txt`**, **`runtime.txt`**, **`Procfile`**, **`railway.json`** (Flask web), and **`railway.worker.json`** (duplicate the repo into a second service and point **Config file** at `railway.worker.json` for `python main.py --mode paper`). Bind the dashboard with **`PORT`** (set automatically on Railway). Share one SQLite path via **`QUANTBOT_DB_PATH`** on a **mounted volume** (or Postgres) so the worker and web process see the same `trades` / `portfolio_state` rows.
