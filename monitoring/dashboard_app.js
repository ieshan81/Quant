(function () {
  ...
  function pickCanonicalAccount(mc) {
    if (!mc) return { equity: null, cash: null, buying_power: null, source: null };
    var ct = mc.canonical_truth && mc.canonical_truth.account_state;
    var ca = mc.canonical_account;
    var ac = mc.account || {};
    var tl = mc.topline || {};
    var src = (ct && (ct.primary_source || ct.source)) || (ca && ca.primary_source) || ac.account_source || tl.account_source;
    function pick(field) {
      if (ct && ct[field] != null) return ct[field];
      if (ca && ca[field] != null) return ca[field];
      if (ac[field] != null) return ac[field];
      return tl[field] != null ? tl[field] : null;
    }
    return {
      equity: pick("equity"),
      cash: pick("cash"),
      buying_power: pick("buying_power"),
      source: src
    };
  }

  function _isFetchAbortError(e) {
    var msg = String((e && e.message) || e || "").toLowerCase();
    var name = String((e && e.name) || "").toLowerCase();
    return name === "aborterror" || msg.indexOf("abort") >= 0 || msg.indexOf("aborted") >= 0;
  }

  function updateHeaderStrip(vm, mc) {
    var canon = pickCanonicalAccount(mc);
    var eq = vm && vm.equity != null ? vm.equity : (mc ? canon.equity : null);
    var cash = vm && vm.cash != null ? vm.cash : (mc ? canon.cash : null);
    var bp = vm && vm.buyingPower != null ? vm.buyingPower : (mc ? canon.buying_power : null);
  ...
  function _loadEquityRange(range) {
    ...
    fetchFn("/api/account/history?range=" + encodeURIComponent(range), { headers: _authHeaders(), cache: "no-store" }, _dashFetchCtrl)
      ...
      .then(function (d) {
        ...
        var liveEq = _mcCache
          ? pickCanonicalAccount(_mcCache).equity
  ...
  function _setDashText(id, text, className) {
    var el = _dashEl(id);
    if (!el) return;
  ...
  function renderOverviewTruth(vm) {
    try {
      ...
      _setDashText(
        "ovRecovery",
        cts.capital_recovery_active ? "Active" : (isFiniteNum(bp) && bp < 1 ? "Likely (low BP)" : "Off")
      );
      var flLine =
        safeText(fl.ui_label || fl.execution_mode, "—") +
        " · scored " + safeText(fl.scored_count, "0") +
        "/" + safeText(fl.symbols_scanned, "0");
      if (fl.top_rejected_reason || cts.fast_loop_top_reject) {
        flLine += " · top reject: " + safeText(fl.top_rejected_reason || cts.fast_loop_top_reject, "—");
      }
      _setDashText("ovFastLoop", flLine);
      var liveAllowed = cts.live_allowed;
      _setDashText(
        "ovLiveAllowed",
        liveAllowed === true ? "Allowed" : liveAllowed === false ? "Blocked" : "—"
      );
      var blockers = cts.live_blockers || cts.momo_blockers || [];
      var btxt = blockers.length
        ? "Blockers: " + blockers.map(function (b) { return String(b).replace(/_/g, " "); }).join(", ")
        : "No architecture blockers reported.";
      _setDashText("ovBlockers", btxt);
      _setDashText("ovMomoMemo", cts.momo_summary || cts.capital_alert || "MoMo memo available on Mission Control / GPT bundle.");
    } catch (eOv) {
      console.error("[renderOverviewTruth]", eOv);
    }
  }
  ...
  function renderOverview(vm) {
    ...
    var hintEl = _dashEl("overviewDataHint");
    if (hintEl) {
      if (hint) {
    ...
    _setDashText("mMode", vm.mode ? String(vm.mode).toUpperCase() : "—");

    var pnlD = _dashEl("mPnlD");
    var pnlP = _dashEl("mPnlP");
    _setDashText("mEq", vm.equity != null ? fmtMoney(vm.equity) : "—");
    if (pnlD) {
    ...
    _setDashText("mCash", isFiniteNum(vm.cash) ? fmtMoney(vm.cash) : "—");
    var mo = vm.marketOpen;
    _setDashText("mMkt", mo === true ? "OPEN" : mo === false ? "CLOSED" : "—");
    var st = vm.capitalStage || {};
    var stageText = st.stage != null ? String(st.stage) : (st.name != null ? String(st.name) : "—");
    _setDashText("mCap", stageText.toUpperCase());

    renderOverviewTruth(vm);
    ...
    if (tb) {
      var posTopEmpty = _dashEl("posTopEmpty");
      if (posTopEmpty) posTopEmpty.style.display = top.length ? "none" : "block";
    ...
    var decEmpty = _dashEl("decEmpty");
    if (decEmpty) decEmpty.style.display = decs.length ? "none" : "block";
    ...
    if (dt) {
      ...
      console.error("[renderOverview]", eRenderOv);
    }
  }
  ...
  function paintViewModel(vm, fromCache) {
    ...
    var done = function () {
      try { renderOverview(vm); } catch (e1) { console.error("[paintOverview]", e1); }
      try { renderPositionsTab(vm); } catch (e2) { console.error("[paintPositions]", e2); }
      try { renderActivity(vm); } catch (e3) { console.error("[paintActivity]", e3); }
    };
  ...
  function bindTabs() {
    ...
    function show(name) {
      ...
      if (name === "overview" && window.__lastVm) {
        try { renderOverview(window.__lastVm); } catch (eTab) { console.error(eTab); }
      }
    }
    var t;
  ...
  function renderBrokerTransitionCard(data) {
    data = data || {};
    var firstRun = data.first_run_baseline_required === true || data.wizard_state === "FIRST_RUN_BASELINE_REQUIRED";
    var headline = document.getElementById("btWizardHeadline");
    var st = document.getElementById("btWizardState");
    var op = document.getElementById("btOperatorLabel");
    ...
    var raw = document.getElementById("btPreviewRaw");
    var mach = document.getElementById("btMachineType");
    var cbox = document.getElementById("btConfirmBox");
    var cphrase = document.getElementById("btConfirmPhrase");
    var fp = data.broker_fingerprint || {};
    var opLabel = data.operator_label || (firstRun ? "First baseline required" : safeText(data.transition_type, "—"));

    if (headline) {
      headline.textContent = firstRun
        ? "First fingerprint baseline required"
        : safeText(data.operator_label, "Broker account transition");
    }
    if (st) {
    ...
    if (ac) {
      var epochAudit = (data.active_epoch || {}).acceptance_audit_result;
      var auditSt = typeof epochAudit === "object" ? epochAudit.acceptance_status : epochAudit;
      ac.textContent = safeText(data.acceptance_status || auditSt, "—");
    }
    ...
    if (cfg) {
      cfg.textContent =
        ...
        (firstRun ? " · No previous fingerprint on file" : "");
    }
    if (cbox && cphrase) {
      cbox.style.display = "block";
      cphrase.textContent = safeText(data.required_confirmation, "RESET PAPER RUNTIME");
    }
    ...
    _fillBtList("btBrokerSnapList", [
      ...
      "Base URL: " + safeText(snap.base_url, fp.base_url),
      "Equity: $" + safeText(snap.equity, fp.equity),
      "Buying power: $" + safeText(snap.buying_power, fp.buying_power),
      "Positions: " + safeText(snap.positions_count, fp.positions_count),
      "Open orders: " + safeText(snap.open_orders_count, fp.open_orders_count),
      "Fingerprint: " + safeText(snap.fingerprint_hash_short, (fp.fingerprint_hash || "").slice(0, 12))
    ]);
    if (raw) {
    ...
    if (mach) {
      mach.textContent = "Machine type: " + safeText(data.transition_type_machine || data.transition_type, "—");
    }
    _btPreviewCache = data;
    _btPreviewLoaded = true;
  }

  var _btPreviewLoaded = false;
  ...
  function _mcRenderCryptoScanner(d) {
    ...
    var scanned = diag.symbols_scanned_this_cycle != null ? diag.symbols_scanned_this_cycle : diag.symbols_scanned;
    var universe = diag.universe_count != null ? diag.universe_count : (diag.broker_supported_count != null ? diag.broker_supported_count : null);
    var scoredN = diag.scored_count;
    var panelMsg = diag.scanner_panel_message || "";
    var provider = diag.provider_status || diag.universe_source || "—";
    var lastScan = diag.last_scan_at || diag.last_cycle_at || "";
    var topRej = diag.top_rejected_reason || "";
    ...
    if (!topRej && tops.length && tops[0].reject_reason) topRej = tops[0].reject_reason;
    var stats =
      ...
      "<span>Scored <strong>" + esc(String(scoredN != null ? scoredN : "—")) + "</strong></span>" +
      (th != null ? "<span>Threshold <strong>" + esc(String(th)) + "</strong></span>" : "") +
      "</div>" +
      '<div class="mc-scanner-meta muted" style="font-size:11px;margin:4px 0">' +
      "Provider <strong>" + esc(String(provider)) + "</strong>" +
      (lastScan ? " · Last scan " + esc(fmtTimeShort(lastScan)) : "") +
      (topRej ? " · Top reject " + esc(String(topRej).replace(/_/g, " ")) : "") +
      "</div>";
    ...
    var scanHtml =
      ...
      (rows
        ...
        : '<span class="muted">' + esc(panelMsg || "No scored candidates this cycle.") + "</span>");
  ...
  function _mcRenderCommandStrip(d) {
    ...
    var t = d.topline || {};
    var ac = d.account || {};
    var canonAcct = pickCanonicalAccount(d);
    if (canonAcct.equity != null) {
      ac = { equity: canonAcct.equity, cash: canonAcct.cash, buying_power: canonAcct.buying_power, account_source: canonAcct.source };
      t = { equity: canonAcct.equity, cash: canonAcct.cash, buying_power: canonAcct.buying_power };
    }
    var mi = d.mission || {};
    ...
    if (w.worker_first_cycle_pending) {
      workerLab = "Fresh";
      workerSub = w.status_message || "Fresh — waiting for first successful worker cycle";
    } else if (w.status_message && fresh) {
      workerSub = String(w.status_message);
    ...
    var pushBlock = cfl.fast_loop_display_blocker || cfl.exact_push_blocker;
  ...
  function _mcRenderCockpitMain(d) {
    ...
    if (canonAcct.equity != null) {
      ac = Object.assign({}, ac, {
        equity: canonAcct.equity,
        cash: canonAcct.cash,
        buying_power: canonAcct.buying_power
      });
    }
  ...
  function _mcTopline(d) {
    ...
    if (_mcEquitySeriesCache && mcEquityChart) {
      var liveEq = pickCanonicalAccount(d).equity;
    ...
    var cards = [
      ...
      },
      {
        ...
        render: function () {
          ...
          if (w.worker_first_cycle_pending) {
            return (w.status_message || "Fresh — waiting for first successful worker cycle") + " Session: " + sess + ".";
          }
      ...
      },
      {
        ...
        render: function () {
          ...
          if (rec && rec.enabled && rec.human_summary) {
  ...
  function _mcLoadEquityChart(range) {
    ...
    fetchFn("/api/account/history?range=" + encodeURIComponent(_mcEqRange), { cache: "no-store", headers: _authHeaders() }, _mcFetchCtrl)
      ...
      .then(function (d) {
        ...
        var liveEq = _mcCache ? pickCanonicalAccount(_mcCache).equity : null;
        series = _mergeLiveEquityIntoSeries(series, liveEq);
      ...
      .catch(function (e) {
        ...
        var errMsg = safeText(e && e.message, String(e));
        if (hint) {
          hint.textContent = "Equity chart unavailable: /api/account/history — " + errMsg;
        }
      });
  }
