...
def sentiment_for_symbol(symbol: str) -> tuple[float, float, dict[str, Any]]:
    ...
    """
    from data.sentiment_feed import aggregate_sentiment_score

    raw, meta = aggregate_sentiment_score(symbol)
...
def format_sentiment_report(symbol: str) -> str:
    ...
    lines = [
        ...
        f"Sentiment aggregate (news FinBERT + social RoBERTa): {raw:+.4f}",
