...
_EXTRA_BOT_DEFAULTS: dict[str, tuple[float, str]] = {
    ...
    "telegram_momo_chat_enabled": (0.0, "1=enable Momo Telegram command polling"),
    "telegram_important_updates_enabled": (1.0, "1=send important Momo Telegram updates"),
    "telegram_daily_summary_enabled": (1.0, "1=send daily Telegram summary"),
    "telegram_critical_only_mode": (0.0, "1=only critical Telegram alerts"),
    "reset_paper_on_startup": (0.0, "1=wipe paper state on next worker boot (dangerous)"),
    "wipe_ghost_positions": (0.0, "1=remove ghost SQLite positions on reconcile"),
    "dynamic_account_sizing_enabled": (1.0, "1=dynamic account profile from live equity"),
...
_BOT_KEY_DESCRIPTIONS: dict[str, str] = {
    ...
    "crypto_fast_loop_enabled": "1=run independent 10-30s crypto push/pull loop (paper until live readiness)",
    "crypto_fast_loop_cycle_seconds": "Seconds between fast crypto loop iterations",
    "crypto_fast_loop_max_positions": "Max open crypto positions for fast loop entries",
    "crypto_fast_loop_max_total_crypto_allocation_pct": "Max total crypto sleeve pct for fast loop",
    "crypto_fast_loop_max_single_position_pct": "Max single crypto position pct for fast loop",
    "crypto_fast_loop_min_score": "Min score for fast loop push candidates",
    "crypto_fast_loop_take_profit_pct": "Fast loop take-profit fraction",
    "crypto_fast_loop_stop_loss_pct": "Fast loop stop-loss fraction",
    "crypto_fast_loop_trailing_pullback_pct": "Fast loop trailing pullback fraction",
    "crypto_fast_loop_max_spread_pct": "Max spread pct for fast loop entries",
    "crypto_fast_loop_cooldown_seconds": "Re-entry cooldown seconds for fast loop",
    "crypto_fast_loop_daily_trade_limit": "Max fast loop trades per day",
    "crypto_fast_loop_min_notional": "Min order notional USD for fast loop",
